...
### 8.9 两张大的表进行join，产生数据倾斜

这样的话可以将数据量较小的那张表的join的字段进行扩容操作，一定程度上增加数据量，，然后大的表中的join字段随机打散。
...
1. **一个是对于可视化平台这块的，在日后的需求迭代中，有没有可能会扩展到其他平台？**
   ...
   - 针对于第一点，我这里独自做了一个集齐滴滴内部所有可视化平台的流量数据的统一性建设，将埋点数据进行抽取，隔离出一张可视化全平台的报表流量DWD数据明细层。
   - 每类报表为其分配了特定的id进行区分，report_type

2. **第二点是有没有可能会扩展到除了乘客方向的其他业务方向？**

   - 这里需要说明 一点，每个平台配置好的报表会统一的配置到一个叫做万花筒的门户下，但是这个门户下没有相应报表的维度信息

   - 其他方向包括

     > 司机；特惠月；网开台，MP，UT，拼车，安全，决策，乘客等9个方向

   - **短期方案**，是每个业务方向的报表维度信息会汇总到2个产品姐姐那边，然后最后会统一的按照我的dim层设计的字段，统一维护到线上的一个文档中，然后每天以全量的方式导入至dim维度层。

   - **长期方案**，产品方与万花筒平台的后台负责方进行报表维度数据的落表，这样就不需要我们手动来维护整个报表

#### 二期需求

从数易的可视化平台扩展到数易、报表王、定制化这3个可视化平台，业务方向从1个乘客方向，慢慢扩展到了：乘客，司机，特惠月，网开台，MP，UT，拼车、决策、安全等9个业务方向。



### 2.2 全生态数据增长建设

需求：**网约车在微信，支付宝，QQ等生态下均有相应的业务**
