...
3. <font color=green>二进制日志只在每次事务提交的时候一次性写入缓存中的日志"文件"(对于非事务表的操作，则是每次执行语句成功后就直接写入)</font>。而redo log在数据准备修改前写入缓存中的redo log中，然后才对缓存中的数据执行修改操作；而且保证在发出事务提交指令时，先向缓存中的redo log写入日志，写入完成后才执行提交动作。
...
在数据修改的时候，不仅记录了**redo**，还记录了相应的**undo**，如果因为某些原因导致事务失败或回滚，可以借助该undo进行回滚。

undo log 和 redo log记录物理日志不一样，它是逻辑日志。<font color=red>可以认为当delete一条记录时，undo log中会记录一条对应的insert记录，反之亦然，当update一条记录时，它记录一条对应相反的update记录。</font>但是实际并不是这样记录的。
...
- <font color=red>DATA_TRX_ID</font> 标记了最新更新这条行记录的transaction id，每处理一个事务，其值自动+1

- <font color=red>DELETE_BIT </font>用于标识该记录是否被删除，这里的不是真正的删除数据，而是标志出来的删除。真正意义的删除是在commit的时候。
...
客户端发起写请求到NameNode，NameNode返回可用的资源，客户端根据资源使用情况对要写入的数据分块，逐一上传块到DataNode，DataNode获取上传块数据并写入磁盘，完成后报告给NameNode块信息，同时也告诉客户端写入成功，客户端继续后续块的写入，在此期间NameNode接受到DataNode块写入完成信息之后备份数直到满。
...
1. **MapTask**收集Mapper中的map()方法每次输出的key-value值，放入到环形缓冲区中。
...
1. <font color=gree>作业提交</font>
   1. Client调用job.waitForCompletion方法，向整个集群提交MapReduce作业。
   2. Client会向RM申请一个**Application**
   3. RM给Client返回该job**资源的提交路径**和**作业id**
   4. Client提交**jar包**，**切片信息**和**配置文件**到指定的资源提交路径上。
   5. Client提交完资源后，向RM申请运行**ApplicationMaster**。ApplicationMaster运行在刚刚申请的Container中。
2. <font color=gree>作业初始化</font>
   6. 当RM收到Client的请求后，将请求封装成**task**，将该job添加到**容量调度器中**
   7. 某个空闲的NM领取到该Job，创建Container，并产生**ApplicationMaster**
   8. 下载Client提交的资源到NM本地
3. <font color=gree>任务分配</font>
   9. **ApplicationMaster**向RM申请运行多个**MapTask**任务资源。
   10. RM将运行**MapTask**任务分配给另外两个**NodeManager**，另外两个**NodeManager**分别领取到任务并创建容器。
4. <font color=gree>任务运行</font>
   11. AM向接收到任务的**NodeManager**发送程序启动脚本，这两个**NodeManager**分别启动**MapTask**，**MapTask**对数据分区排序。
   12. **ApplicationMaster**等待所有**MapTask**运行完毕后，向RM申请容器，运行ReduceTask
   13. **ReduceTask**向**MapTask**获取相应分区的数据。
   14. 程序运行完毕后，MR会向RM申请注销自己。
5. <font color=gree>进度和状态更新</font>
   15. Yarn中的任务将其进度和状态（包括counter）返回给应用管理器，客户端每秒向应用管理器请求进度更新，展示给用户。
6. <font color=gree>作业完成</font>
...
每碰到一个Action就会产生一个job，每个job开始计算的时候，总是从这个job最开始的RDD开始计算。

每个job总是从它血缘的开始计算，难免会有计算过程重复执行的情况。比如（中间过程产生了一些列的RDD，最终有两个action的时候，这两个action依赖同一个RDD，这样每次执行action的时候，就会重复计算）

如果整个RDD依赖的DAG图的血缘关系过长，就很可能出现分区数据损坏或丢失，则又要从头开始计算来达到容错的目的。

<font color=red>每个 job 都会重新进行计算, 在有些情况下是没有必要, 如何解决这个问题呢?</font>

Spark 一个重要能力就是可以持久化数据集在内存中. 当我们持久化一个 RDD 时, 每个节点都会存储它在内存中计算的那些分区, 然后在其他的 action 中可以重用这些数据. 这个特性会让将来的 action 计算起来更快(通常快 10 倍). 对于迭代算法和快速交互式查询来说, 缓存(**Caching**)是一个关键工具.
...
2. 持久化的数据丢失可能性更大，磁盘，内存都有可能存在数据丢失的情况。但是`checkpoint`的数据通常是存储在如**HDFS等容错、高可用的文件系统，数据丢失的可能性较小。**
...
1. 边界数组的长度小于等于128
   - 这个时候，每来一个key，轮询的方式遍历数组中的每个元素，然后确定分区号
2. 边界数组的长度大于128的时候
   - 通过**二分查找**来快速定位分区号。
...
### Spark SQL Text 转化为实际的物理任务的流程

![在这里插入图片描述](https://img-blog.csdnimg.cn/20200518170149838.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2E1Njg1MjYz,size_16,color_FFFFFF,t_70#pic_center)

1. SQL语句经过**SqlParse（解析器）**解析成**Unresolved LogicalPlan**
2. **分析器Analyzer**结合**数据字典Catalog**进行绑定，生成**Resolved LogicalPlan**
3. **优化器Optimizer**对**Resolved LogicalPlan**进行优化，生成优化后的**Optimizer LogicalPlan**
4. **SparkPlan**将上述的**Optimizer LogicalPlan**转换成**PhysicPlan**
5. 使用**prepareForExecption**将**PhysicPlan**转换成可执行的物理计划
6. 使用**execute()**执行可执行的物理计划，生成**DataFrame**



### Spark SQL 源码分析（version：Spark2 && Spark3.1.2）

> 补充时间：2022年4月30日
>
> 备注： 详细了解一下Spark SQL的部分关键源码，明白一条 SQL 语句是如何一步步转换成最终的RDD任务

#### 前置知识：Antlr4 语法生成器工具

Spark SQL内部是通过第三方插件Antlr进行SQL语句的语法分析的。在了解Spark SQL源码之前，首先要了解Antlr的相关知识。

[Antlr4 GitHub doc地址](https://github.com/antlr/antlr4/blob/master/doc/index.md)

[Spark SQL中使用的SQL语法文件：sqlBase.g4文件](https://github.com/apache/spark/tree/master/sql/catalyst/src/main/antlr4/org/apache/spark/sql/catalyst/parser)



#### Spark2源码分析

Spark SQL模块在处理SQL时，内部是通过一个名为`Catalyst`的优化器（我更喜欢称之为：处理器）将SQL转换成最终的逻辑执行计划。
...
Spark RDD通过其Transactions操作，形成了RDD血缘关系图DAG，最后通过Action的调用，发出Job并调执行。
...
**TaskSetManager**负责监控管理同一个stage中的Tasks，TaskScheduler就是以TaskSetManager为单元来调度任务。包括**任务推断**、**Task本地化调度**，并对**Task进行资源分配**。
...
2. 不是聚合类的`shuffle`算子（<font color=red>没有预聚合功能的</font>，比如：groupByKey）

该过程的磁盘写机制其实跟未经优化的`HashShuffleManager`是一模一样的，因为都要创建数据量惊人的磁盘文件，只是在最后一次溢写，进行了一次磁盘文件的合并而已。因此少量的最终磁盘文件，也让该机制相对未经优化的HashShuffleManager来说，Shuffle read的性能会更好。（**同时也是采用缓冲区溢写的方法落盘小文件，解决了HashShuffle中内存的问题**）
...
在 Spark 任务运行过程中，如果 shuffle 的map端处理的数据量比较大，但是map端缓冲的大小是固定的，可能会出现map端缓冲数据频繁**spill溢写到磁盘**文件中的情况，使得性能非常低下，通过调节map端缓冲的大小，可以避免频繁的磁盘 IO 操作，进而提升 Spark 任务的整体性能。
...
### 8.8 更换分区器（RangePartitioner）



### 8.9 两张大的表进行join（无法广播），产生数据倾斜
...
1. **对数据倾斜的那个RDD，使用sample算子采样出一份样本数据，统计下每个key的数量，看看导致数据倾斜的数据量最大的是哪几个key。**
2. 然后将这几个key对应的数据从原来的RDD中拆分出来，形成一个单独的RDD，并给**每个key**都打上`0~n`以内的随机前缀；不会导致数据倾斜的大部分key形成另外一个RDD。
3. 接着将join的另一个RDD过滤出来2中所提取出来的那些导致数据倾斜的key，也单独形成一个RDD。这个RDD中的key是少量的，将每条数据膨胀成n倍，这n条数据都按照顺序添加一个`0~1`的前缀；那些不会导致数据倾斜的key单独形成另外一个RDD。
4. 再将随机附加了随机前缀的RDD与另一个膨胀了n倍的RDD进行join，这样就可以将原先相同的key打散成n分，分散到多个task中去进行join
5. 而另外两个普通的RDD就照常join即可
6. 最后将两次join的结果使用union算子合并起来即可，最终得到join的结果。

> 方案优点

对于两个大RDD进行join时的数据倾斜，如果只是某一个key导致了数据倾斜，采用该方式可以用最有效的方式打散key进行join。而且只需要针对少数倾斜key对应的数据进行扩容n倍，不需要对全量数据进行扩容，避免了占用过多内存。

> 方法局限性

如果导致倾斜的key特别多的话，比如成千上万个key都导致数据倾斜了，就不能使用本解决方案了。



[Spark数据倾斜的解决方案：参考](https://blog.csdn.net/meihao5/article/details/81084876)
...
![image-20210917191421213](source/2021%E5%B9%B48%E6%9C%88%E4%BB%BD%E7%A7%8B%E6%8B%9B%E5%A4%8D%E4%B9%A0%E7%AC%94%E8%AE%B0/image-20210917191421213-16318772632891.png)
...
参与join的两张表是`item`和`order`，join key分别是`item.id`以及`order.id`，假设这个Join采用的是Hash Join算法，整个过程会经历三步：

![img](source/2021%E5%B9%B48%E6%9C%88%E4%BB%BD%E7%A7%8B%E6%8B%9B%E5%A4%8D%E4%B9%A0%E7%AC%94%E8%AE%B0/webp-16318778783212-16318778797984.webp)

1. 确定**Build Table**（映射表、小表）以及**Probe Table**（探查表、大表）。其中**Build Table**用于构建**Hash Table**，而**Probe**会遍历自身所有**Key**，映射到所生成的**Hash Table**上去匹配。
2. **Build Table**构建**Hash Table**。依次读取**Build Table**（Item）的数据，对于每一行数据根据Join的**key**（item.id）进行**hash**，**hash**到对应的**Bucket**，生成**hash table**中的一条记录。数据缓存在内存中，如果内存放不下需要**dump**到外存。
3. **Probe Table**探测。依次扫描**Probe Table**（order）的数据，使用相同的**hash函数**进行映射，如果成功**映射到Hash Table**之后，再**检查join条件**（item.id=order.id），如果匹配成功就可以将两者join在一起。

**补充点**

> 1. Hash Join 的性能。从上面的原理图可以看出，hash join对两张表基本只扫描依次，**算法效率是O(a+b)**，比起蛮力的笛卡尔积算法的a*b快了很多数量级。
> 2. Build Table要尽量选择小表：
>    - 从原理上可以看出，构建的Hash Table是需要被频繁访问的，所以**Hash Table最好能全部加载到内存中**
>    - 这也决定了Hash Table只适合至少一个小表join的场景

上述介绍的Hash join 是在单机版中运行的join算法，而在大数据分布式情况下，目前成熟的有两套Hash Join算法：``broadcast hash join` 和 `shuffle hash join`

#### Broadcast Hash Join

`broadcast hash join`是将其中一张小表广播分发到另一张大表所在的分区节点上，分别并发地与上的分区记录进行`hash join`。broadcast适用于小表很小，可以直接广播的场景。

需要注意的是，Spark中对于可以广播的小表，默认限制是10M以下。（参数是`spark.sql.autoBroadcastJoinThreshold`）

![img](source/2021%E5%B9%B48%E6%9C%88%E4%BB%BD%E7%A7%8B%E6%8B%9B%E5%A4%8D%E4%B9%A0%E7%AC%94%E8%AE%B0/webp-16318798352397-16318798366029.webp)

在执行上，主要可以分为以下两步：

1. broadcast阶段：将小表广播分发到大表所在的所有主机。分发方式可以是driver分发，或者是p2p的方式。
2. hash join阶段：在每个executor上执行单机版的hash join，**小表映射，大表试探**
...
`Shuffle Hash Join`按照join key进行分区，根据**key相同必然分区相同的原理**，将大表join分而治之，划分为小表的join，充分利用集群资源并行化执行。

![img](source/2021%E5%B9%B48%E6%9C%88%E4%BB%BD%E7%A7%8B%E6%8B%9B%E5%A4%8D%E4%B9%A0%E7%AC%94%E8%AE%B0/webp-163188039067910-163188039254812.webp)

1. **Shuffle阶段**
   - 分别将两个表按照join key进行分区，将相同join key的记录重分布到同一节点，两张表的数据会被分布到集群中所有节点。这个过程称为shuffle
2. **Hash Join阶段**
   - 每个分区节点上的数据单独执行单机hash join算法

如果两张小表join，可以直接使用单机版的hash join；如果一张大表join一张极小表，可以选择broadcast hash join算法；而如果是一张大表join一张小表，小表不能广播超过10M了，则可以选择shuffle hash join算法；
...
**没有使用HashJoin，即单机join的时候，没有HashTable的生成。**

Spark SQL对两张大表join采用了全新的算法：Sort-Merge Join，整个过程分为三步步骤：

![img](source/2021%E5%B9%B48%E6%9C%88%E4%BB%BD%E7%A7%8B%E6%8B%9B%E5%A4%8D%E4%B9%A0%E7%AC%94%E8%AE%B0/webp-16319326499621-16319326513313.webp)
...
2. **Sort阶段**
   - 对单个分区节点的两表数据，分别进行排序
3. **Merge阶段**
   - 对排好序的两张分区表数据执行join操作。join操作很简单，分别遍历两个有序序列，碰到相同join key就merge输出，否则取更小一边，（<font color=red>类似于双指针合并两个有序序列</font>）,如下图所示：
   - ![img](source/2021%E5%B9%B48%E6%9C%88%E4%BB%BD%E7%A7%8B%E6%8B%9B%E5%A4%8D%E4%B9%A0%E7%AC%94%E8%AE%B0/webp-16319328669854-16319328684116.webp)

仔细分析可以发现，Sort-Merge Join的单价并不比Shuffle hash join小，反而是多了排序。**那么为什么SparkSQL还会在两张大表的场景下选择使用Sort-Merge Join算法呢？**

当Spark采用**Sort Shuffle**时，在经过Shuffle之后，Partition的数据都是按照key排序的，因此理论上可以认为数据经过Shuffle之后是不需要Sort的，可以直接Merge。



### Join时通过BloomFilter布隆过滤器进行 进行谓词下推



谓词下推是将过滤条件从计算进程下推到存储进程先行执行，注意这里有两种类型进程：计算进程以及存储进程。计算与存储分离思想，这在大数据领域相当常见，比如最常见的计算进程有SparkSQL、Hive、impala等，负责SQL解析优化、数据计算聚合等，存储进程有HDFS（DataNode）、Kudu、HBase，负责数据存储。正常情况下应该是将所有数据从存储进程加载到计算进程，再进行过滤计算。谓词下推是说将一些过滤条件下推到存储进程，直接让存储进程将数据过滤掉。这样的好处显而易见，过滤的越早，数据量越少，序列化开销、网络开销、计算开销这一系列都会减少，性能自然会提高。

> 在逻辑执行计划优化层面，比如SQL语句：select * from order ,item where item.id = order.item_id and item.category = ‘book’，正常情况语法解析之后应该是先执行Join操作，再执行Filter操作。通过谓词下推，可以将Filter操作下推到Join操作之前执行。即将where item.category = ‘book’下推到 item.id = order.item_id之前先行执行。
...
![image-20210922152340903](source/2021%E5%B9%B48%E6%9C%88%E4%BB%BD%E7%A7%8B%E6%8B%9B%E5%A4%8D%E4%B9%A0%E7%AC%94%E8%AE%B0/image-20210922152340903-16322954231991.png)

```shell
spark-submit.cmd脚本下最终会执行一个指令

java org.apache.spark.deploy.SparkSubmit 执行
```

**SparkSubmit.scala**

![image-20210922155639198](source/2021%E5%B9%B48%E6%9C%88%E4%BB%BD%E7%A7%8B%E6%8B%9B%E5%A4%8D%E4%B9%A0%E7%AC%94%E8%AE%B0/image-20210922155639198-16322974007352.png)



![image-20210922155810394](source/2021%E5%B9%B48%E6%9C%88%E4%BB%BD%E7%A7%8B%E6%8B%9B%E5%A4%8D%E4%B9%A0%E7%AC%94%E8%AE%B0/image-20210922155810394-16322974914213.png)
...
```scala
  def doSubmit(args: Array[String]): Unit = {
    ...
    appArgs.action match {  // 根据解析好的参数对象的action属性值，匹配具体的动作：是submit，还是kill，还是requestStatus，还是printVersion
...
![image-20210922160232506](source/2021%E5%B9%B48%E6%9C%88%E4%BB%BD%E7%A7%8B%E6%8B%9B%E5%A4%8D%E4%B9%A0%E7%AC%94%E8%AE%B0/image-20210922160232506-16322977552994.png)

parse方法具体解析`spark-sumbit`脚本传过来的参数

![image-20210922160803410](source/2021%E5%B9%B48%E6%9C%88%E4%BB%BD%E7%A7%8B%E6%8B%9B%E5%A4%8D%E4%B9%A0%E7%AC%94%E8%AE%B0/image-20210922160803410-16322980847205.png)
...
private def submit(args: SparkSubmitArguments, uninitLog: Boolean): Unit = {
    def doRunMain(): Unit = {
        if (args.proxyUser != null) {
            ...
            runMain(args, uninitLog)   // 执行主程序
        }
    }
    
    
    private def runMain(args: SparkSubmitArguments, uninitLog: Boolean): Unit = 
    	// 通过准备提交环境方法，模式匹配，返回四个参数，其中重要的是childMainClass
		...
		// 判断childMainClass反射出的类是否继承了SparkApplication类，如果继承了直接返回该类的示例，如果没有继承，直接创建mainClass
...
// In yarn-cluster mode, use yarn.Client as a wrapper around the user class
    ...
    try {
      app.start(childArgs.toArray, sparkConf)  // org.apache.spark.deploy.yarn.YarnClusterApplication的start方法
    } catch {
      case t: Throwable =>
        throw findCause(t)
    }
...
从上述代码中可以看出，最终`childMainClass=org.apache.spark.deploy.yarn.YarnClusterApplication`
...
private[spark] class YarnClusterApplication extends SparkApplication {
  ...
  override def start(args: Array[String], conf: SparkConf): Unit = {
    ...
    new Client(new ClientArguments(args), conf, null).run() // 创建一个Client，并执行run方法
  }

}
	// amClass集群模式和Client模式下具体的取值
    val amClass =
      if (isClusterMode) {
...
ResourceManager接收命令，在可用的节点上启动`ApplicationMaster`
...
```scala
  def main(args: Array[String]): Unit = {
    ...
    master = new ApplicationMaster(amArgs, sparkConf, yarnConf)  // 创建ApplicationMaster
	...
	// master.run()  ApplicationMaster运行
	...
	// 集群模式下，运行runDriver()
      if (isClusterMode) {
        runDriver()
          ...
          // 如果是Client模式运行 runExecutorLauncher()方法
        runExecutorLauncher()
      }
    } catch {
      case e: Exception =>
...
```scala
  private def runDriver(): Unit = {
    ...
    try {
        // 等待上限文环境初始化成功后才可以往下运行：SparkContext
      ...
      if (sc != null) {
        ...
        val port = userConf.get(DRIVER_PORT)
          //ApplicationMaster 向ResourceManager进行注册（目的：通信，申请资源）
        registerAM(host, port, userConf, sc.ui.map(_.webUrl), appAttemptId)

        val driverRef = rpcEnv.setupEndpointRef(
          RpcAddress(host, port),
          YarnSchedulerBackend.ENDPOINT_NAME)
          // ApplicationMaster向RM注册成功之后，RM会通过client给ApplicationMaster分配可用的资源
...
```scala
  private def startUserApplication(): Thread = {
    ...
    if (args.primaryRFile != null &&
      ...
      // 使用类加载器加载用户class，这个class就是Spark-submit时所指定的具体的class， 获取main方法
    ...
    userThread.setName("Driver")  // 此处说明ApplicationMaster中创建了Driver子线程
    userThread.start()
    userThread
  }
```

`createAllocator`，RM向AM分配资源
...
```scala
  private def createAllocator(
	...
	//通过client，RM向ApplicationMaster分配可用的资源
...
```scala
  ...
  def startContainer(): java.util.Map[String, ByteBuffer] = {
	...
	// 准备命令
    val commands = prepareCommand()  // 封装命令，并向相应的Container所在机器上 发送YarnCoarseGrandExecutorBackend
...
AM封装指令`bin/java org.apache.spark.executor.YarnCoarseGrainedExecutorBackend`到Container所在资源中，启动YarnCoarseGrainedExecutorBackend。

![spark1源码](source/2021%E5%B9%B48%E6%9C%88%E4%BB%BD%E7%A7%8B%E6%8B%9B%E5%A4%8D%E4%B9%A0%E7%AC%94%E8%AE%B0/spark1%E6%BA%90%E7%A0%81-16328806267071.png)
...
> **AIO：异步非阻塞式IO**
>
> Linux对AIO支持不够好，Windows对AIO支持的好
>
...
Spark基于Netty新的Rpc框架，借鉴了Akka中的设计，基于Actor模型。

![img](source/2021%E5%B9%B48%E6%9C%88%E4%BB%BD%E7%A7%8B%E6%8B%9B%E5%A4%8D%E4%B9%A0%E7%AC%94%E8%AE%B0/webp-16328829051882-16328829071034.webp)



> 每个Endpoint（Client/Master/Worker）有**1个InBox收件箱**和**N个OutBox发件箱**（N>=1，N取决于当前Endpoint与多少其他的endpoint进行通讯，一个与其通讯的其他Endpoint对应一个OutBox），EndPoint接收到的消息被写入InBox，发送出去的消息写入OutBox并发送其他Endpoint的InBox中



Driver端：`class DriverEndpoint extends IsolatedRpcEndpoint`

Executor端：`class CoarseGrainedExecutorBackend extends IsolatedRpcEndpoint`
...
Driver中执行用户上传的作业，其中首要的是初始化SparkContext对象。
...
其RDD的依赖关系可分为两种：`窄依赖：NarrowDependency`和`宽依赖：ShuffleDependency`

![image-20211002093226904](source/2021%E5%B9%B48%E6%9C%88%E4%BB%BD%E7%A7%8B%E6%8B%9B%E5%A4%8D%E4%B9%A0%E7%AC%94%E8%AE%B0/image-20211002093226904-16331383487631.png)

##### 窄依赖NarrowDependency

> 父RDD的一个或者多个分区只被子RDD的一个分区所使用，不涉及到shuffle



```scala
// 窄依赖是一个抽象类，其具体的实现子类包括两种
@DeveloperApi
abstract class NarrowDependency[T](_rdd: RDD[T]) extends Dependency[T] {
  /**
   * Get the parent partitions for a child partition.
...
对应于两种窄依赖，如下图所示：**OneToOneDependency**

![窄依赖](source/2021%E5%B9%B48%E6%9C%88%E4%BB%BD%E7%A7%8B%E6%8B%9B%E5%A4%8D%E4%B9%A0%E7%AC%94%E8%AE%B0/%E7%AA%84%E4%BE%9D%E8%B5%961.png)



**RangeDependency**

![窄依赖2](source/2021%E5%B9%B48%E6%9C%88%E4%BB%BD%E7%A7%8B%E6%8B%9B%E5%A4%8D%E4%B9%A0%E7%AC%94%E8%AE%B0/%E7%AA%84%E4%BE%9D%E8%B5%962.png)

##### 宽依赖ShuffleDependency

> 父RDD的一个分区被子RDD的多个分区使用，涉及到shuffle

![img](source/2021%E5%B9%B48%E6%9C%88%E4%BB%BD%E7%A7%8B%E6%8B%9B%E5%A4%8D%E4%B9%A0%E7%AC%94%E8%AE%B0/834652-20170505114231242-1674540562.png)
...
在Spark中，使用**转换算子**对RDD进行操作时，会形成RDD的血缘依赖关系；这个依赖关系即为`Dependency`，分为`NarrowDependency`和`ShuffleDependency`两种。这个阶段并不会触发算子的实际执行。RDD中的阶段即根据RDD的血缘关系依赖类型（`ShuflleDependency`）进行Stage的划分的。



#### 2. 阶段的划分

入口是编写的Spark任务中的行动算子，此处是`collect()`算子

```scala
package transformation.singleValue
...
object mapRDD {
	def main(args: Array[String]): Unit = {
		...
		val rdd: RDD[String] = sc.textFile("0")
...
/**
  ...
  def collect(): Array[T] = withScope {
    val results = sc.runJob(this, (iter: Iterator[T]) => iter.toArray)// runJob执行作业
...
//========================= SparkContext.scala ===============================
  ...
  def runJob[T, U: ClassTag](
    ...
    dagScheduler.runJob(rdd, cleanedFunc, partitions, callSite, resultHandler, localProperties.get) // 最终通过DagScheduler执行Job
...
private[scheduler] val eventProcessLoop = new DAGSchedulerEventProcessLoop(this)
  ...
  def runJob[T, U](
    ...
    val waiter = submitJob(rdd, func, partitions, callSite, resultHandler, properties)  // 提交任务
  ...
  def submitJob[T, U](
    ...
    val waiter = new JobWaiter(this, jobId, partitions.size, resultHandler)
      // 核心代码，JobSubmitted会创建一个DAGSchedulerEvent事件，
      // DAGSchedulerEventProcessLoop会通过post方法将上述创建的DAGSchedulerEvent添加到EventQueue事件队列中，等待后续事件的执行
...
/**
   * Put the event into the event queue. The event thread will process it later.
   将事件添加到事件队列中，event线程会之后处理它
   */
def post(event: E): Unit = {
    eventQueue.put(event)
}

// 如何取事件？通过一个子线程eventThread
  ...
  private val eventThread = new Thread(name) {
    ...
    override def run(): Unit = {
      try {
        while (!stopped.get) {
          val event = eventQueue.take() // 通过take方法取出事件
          try {
            onReceive(event) // 核心代码
          } catch {
            case NonFatal(e) =>
              try {
                onError(e)
...
```scala
  ...
  private def doOnReceive(event: DAGSchedulerEvent): Unit = event match { // 模式匹配，匹配出相应类型的Event
    case JobSubmitted(jobId, rdd, func, partitions, callSite, listener, properties) =>
      dagScheduler.handleJobSubmitted(jobId, rdd, func, partitions, callSite, listener, properties) // 通过dagScheduler.handleJobSubmitted方法处理提交的事件
...
/*================================== Stage的划分 =================================== */
 // 此方法会涉及到阶段Stage的划分
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    var finalStage: ResultStage = null  // FinalStage，也叫做ResultStage
    try {
      // New stage creation may throw an exception if, for example, jobs are run on a
      // HadoopRDD whose underlying HDFS files have been deleted.
      finalStage = createResultStage(finalRDD, func, partitions, jobId, callSite) // 创建ResultStage
    } catch {
      case e: Exception =>
  ...
  private def createResultStage(
    ...
    val parents = getOrCreateParentStages(rdd, jobId) // 获取或创建上一阶段，说白了就是父阶段，在此阶段创建完成之后要保证父阶段处理完成
    val id = nextStageId.getAndIncrement()
    val stage = new ResultStage(id, rdd, func, partitions, parents, jobId, callSite)  // 真正创建ResultStage的位置
  ...
  def createShuffleMapStage(shuffleDep: ShuffleDependency[_, _, _], jobId: Int): ShuffleMapStage = {
    ...
    val parents = getOrCreateParentStages(rdd, jobId) // 判断此依赖的RDD是否也存在父阶段，如果有就创建ShuffleMapStage
    val id = nextStageId.getAndIncrement()
...
**<font color=red>Spark中阶段的划分数（Stage数量）== ShuffleDependency（Shuffle依赖）的数量+1</font>**

##### 小总结

1. **DAGScheduler**会将接收到的**Job**封装成**JobSubmitted**事件（实际是DAGSchedulerEvent），加入到eventQueue（事件阻塞队列中）
2. eventThread事件线程的`run()`方法会从阻塞队列中取出相应的event事件
3. 事件取出之后，会经过<font color=green>handleJobSubmitted()</font>方法进行**阶段的划分**，阶段可分为两种类型：
   - **ResultStage（也叫做FinalStage）**
   - **ShuffleMapStage**
4. 在每一个**Stage创建之前**，会通过`getOrCreateParentStages`方法获取或创建该阶段的**父阶段**
5. **getOrCreateParentStages()**方法会根据传入的RDD的依赖类型是否是`ShuffleDependency`进行阶段的划分
6. 阶段的划分是从后向前进行划分的，真正的Job执行，是从前往后进行执行的
7. **Spark的Job中的Stage阶段数=ShuffleDependency的数量+1**
...
```scala
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    try {
      // New stage creation may throw an exception if, for example, jobs are run on a
      // HadoopRDD whose underlying HDFS files have been deleted.
      finalStage = createResultStage(finalRDD, func, partitions, jobId, callSite)  // 此处会划分阶段Stage
    } catch {
      case e: Exception =>
        logWarning("Creating new stage failed due to exception - job: " + jobId, e)
        listener.jobFailed(e)
  ...
  private def submitStage(stage: Stage) {
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
          // getMissingParentStages：获取丢失的父阶段，或者是未提交的阶段
        val missing = getMissingParentStages(stage).sortBy(_.id)
        logDebug("missing: " + missing)
          // 如果missing为空，就代表次阶段没有父阶段了
        if (missing.isEmpty) {
          logInfo("Submitting " + stage + " (" + stage.rdd + "), which has no missing parents")
            // 提交任务  submitMissingTasks
          submitMissingTasks(stage, jobId.get)  // 提交任务的核心方法
        } else {
            // 如果有未提交，或者父阶段，就递归的提交父阶段
          for (parent <- missing) {
            submitStage(parent)  // 此处会有一个递归
          }
          waitingStages += stage
        }
      }
...
上述源码可以看出，阶段划分完成之后，会通过**submitStage()**方法提交最后一个**ResultStage**阶段。

在**submitStage()**方法内部首先会通过**getMissingParentStages**方法找到传过来的Stage是否存在Parent**父阶段**；

- 如果不存在父阶段，就通过**submitMissingTask**方法提交本阶段的任务；
...
/** Called when stage's parents are available and we can now do its task. */
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    val tasks: Seq[Task[_]] = try {
        // 根据Stage的类型：ShuffleMapStage类型；还是ResultStage类型创建不同类型的任务Task
      stage match {
        case stage: ShuffleMapStage =>
          // 分区计算，返回相应的分区id，默认是0 until partitions
          partitionsToCompute.map { id =>
            val locs = taskIdToLocations(id)
            val part = stage.rdd.partitions(id)
              // 创建ShuffleMapTask类型的Task
	...
	// 每个Stage中对应的任务Task创建好之后，会发送给TaskScheduler进行Task任务的调度
    if (tasks.size > 0) {
      logInfo("Submitting " + tasks.size + " missing tasks from " + stage + " (" + stage.rdd + ")")
      stage.pendingPartitions ++= tasks.map(_.partitionId)
      logDebug("New pending partitions: " + stage.pendingPartitions)
        // 每一个阶段创建好的任务Task会被封装成TaskSet对象，通过TaskScheduler.submitTasks将Stage中的任务Tasks传递给TaskScheduler进行任务的调度
...
1. 阶段划分完成之后，**ResultStage**类型的**finalStage**即可创建完成，之后通过**submitStage()**方法进行阶段的提交

2. **submitStage()**接收到**ResultStage**后，首先会尝试获取传入阶段Stage的父阶段：**getMissingParentStages()**

3. 如果parent阶段为空，即传入的阶段**没有父阶段**，就通过**submitMissingTask()**方法来提交Stage中的任务

4. 如果parent阶段不为空，即传入的阶段**还有父阶段没有提交**，就递归的调用**submitStage()**进行父阶段的提交

5. **submitMissingTask()**接收到传入的Stage，会根据传入的Stage的类型创建相应类型的Task
...
6. 任务**Task**创建好之后，会被封装成**TaskSet**对象，通过**TaskScheduler**的**submitTasks()**方法传递给**TaskScheduler**进行任务的调度
...
**TaskScheduler**是一个Trait特质，其子类**TaskSchedulerImpl**是具体的实现类
...
override def submitTasks(taskSet: TaskSet) {
    ...
    this.synchronized {
        // 将接收到的TaskSet进一步封装成TaskSetManager对象
      ...
      if (conflictingTaskSet) {
        ...
        // 将上述构建好的TaskSetManager对象添加到资源调度池中rootPool中（有放就有取，是在backend.reviveOffers()）
        // schedulableBuilder调度构建器，涉及到两种：FIFO和Fair，调度模式默认为FIFO
    ...
    // 此处的调用从资源调度池中取相应的任务（SchedulerBackend 复活，唤醒 Offers）
    backend.reviveOffers()
  }
```

**SchedulerBackend.scala**是一个特质，其子类实现分为集群模式：**CoarseGrainedSchedulerBackend**和本地模式：**LocalSchedulerBackend**
...
```scala
  override def reviveOffers() {
     // SchedulerBackend给自己发送消息，SchedulerBackend在receive方法中接收到消息
    ...
    override def receive: PartialFunction[Any, Unit] = {
      case StatusUpdate(executorId, taskId, state, data) =>
		...
		// 模式匹配，匹配到ReviveOffers类型，最终调用makeOffers方法
      case ReviveOffers =>
        makeOffers()
    ...
    // Make fake resource offers on all executors：翻译：在所有的Executor上创建假的resource offers
    private def makeOffers() {
      // Filter out executors under killing
      val activeExecutors = executorDataMap.filterKeys(executorIsAlive)   // 过滤出目前还存活的Executors
        // 会把存活的Executor封装成workOffers样例类对象，其中包括：Executor的id，Executor的远程Host地址，和Executor的空闲核心数
      val workOffers = activeExecutors.map { case (id, executorData) =>
        new WorkerOffer(id, executorData.executorHost, executorData.freeCores)
      }.toIndexedSeq
        // 最终通过launchTasks方法启动具体的任务
      launchTasks(scheduler.resourceOffers(workOffers))
        
        /*
        	具体从调度池中取任务：scheduler.resourceOffers(workOffers)
        	这里的scheduler就是TaskScheduler任务调度器
        	任务取回之后，会伴随着LaunchTasks消息发送给相应的Executor
        */
    }
      ...
      for (task <- tasks.flatten) {
        ...
        if (serializedTask.limit >= maxRpcMessageSize) {
          ...
          logDebug(s"Launching task ${task.taskId} on executor id: ${task.executorId} hostname: " +
			...
			// 向ExecutorEndPoint发送相应的LaunchTask消息（序列化后的task会被封装成LaunchTask消息）
...
/**
  ...
  def resourceOffers(offers: IndexedSeq[WorkerOffer]): Seq[Seq[TaskDescription]] = synchronized {
    ...
    // Build a list of tasks to assign to each worker.
      // 每个Task和相应的workOffer会被分装成TaskDescription对象，这些TaskDescription对象就是最终要返回的Tasks
    val tasks = shuffledOffers.map(o => new ArrayBuffer[TaskDescription](o.cores))
    val availableCpus = shuffledOffers.map(o => o.cores).toArray
      // 调度池中的任务会根据调度算法进行任务的优先级排序，得到排序好的任务集
    ...
    // NOTE: the preferredLocality order: PROCESS_LOCAL, NODE_LOCAL, NO_PREF, RACK_LOCAL, ANY
      // 任务的本地化调度级别：进程本地化，节点本地化，机架本地化，任意，无偏向
    for (taskSet <- sortedTaskSets) {
      var launchedAnyTask = false
      var launchedTaskAtCurrentMaxLocality = false
        // 任务的本地化级别调度，移动数据不如移动计算
...
1. **TaskScheduler**的**submitTask()**方法接收到**DAGScheduler**传递过来的**TaskSet**，将**TaskSet**进一步封装成**TaskSetManager**，来监控或者失败重试相应的**Task**
2. **TaskSetManager**构建好之后，**TaskScheduler**会将**TaskSetManager**加入到资源调度池**rootPool**中
3. 资源调度池中有放就有取
4. 实际触发取出资源调度池中的任务的方式是**backend.reviveOffers()**
5. **SchedulerBackend**(调度后端)的**reviveOffers()**方法会通过**DriverEndPoint.send(ReviveOffers)**给自己发送消息；**CoarseGrainedSchedulerBackend**接收到消息后根据消息的类型匹配到**ReviveOffer**类型，然后执行**makeOffers()**方法创建**WorkOffer**
6. **makeOffers()**方法中，首先会过滤出目前存活的**Executor**，然后将**Executor**封装成**WorkOffer**对象**workOffers**
7. **workOffers**创建完成后，**TaskScheduler**会按照**FIFO**（默认）或者**Fair**调度算法从资源调度池中取出相应的**TaskSetManager**，进一步取出**TaskSet**中**Task**，结合**workOffers**，按照任务的本地化调度级别，将**Task**和相应的**workOffer**封装成**TaskDescription**对象
8. 接下来通过**launchTask()**方法将**TaskDescription**任务信息发送给远程的**Executor**机器上
9. 在**launchTask()**方法内部，首先会对**TaskDescription**进行序列化，序列化完成后，会将序列化后的数据封装成**LaunchTask**对象消息
10. 最终**LaunchTask**消息会被发送到远程的**ExecutorEndPoint**上，进行任务的执行
...
**CoarseGrainedExecutorBackend**接收到**CoarseGrainedSchedulerBackend**发送来的**LaunchTask**消息。
...
/*======================================  CoarseGrainedExecutorBackend.scala  =======================================*/
  ...
  override def receive: PartialFunction[Any, Unit] = {
	...
	// 接收到消息，并匹配到LaunchTask类型的消息
    case LaunchTask(data) =>
      if (executor == null) {
        exitExecutor(1, "Received LaunchTask command but executor was null")
      } else {
          // 将LaunchTask中的消息进行反序列化，得到相应的TaskDescription
        val taskDesc = ser.deserialize[TaskDescription](data.value)
        logInfo("Got assigned task " + taskDesc.taskId)
          // executor通过launchTask()方法在Executor上发布相应的Task
...
```scala
  ...
  private val runningTasks = new ConcurrentHashMap[Long, TaskRunner]  // 维护正在运行的task列表
  ...
  def launchTask(
      ...
      // 此处的tr实际上是创建一个实现了Runnable接口的线程对象TaskRunner
    val tr = new TaskRunner(context, taskId = taskId, attemptNumber = attemptNumber, taskName,
      serializedTask)
      // 记录正在运行的Task
    runningTasks.put(taskId, tr)
      // 通过线程池来最终执行相应的的Task任务
    threadPool.execute(tr)
  }

/*================================   TaskRunner类继承了Runnable接口    ================================*/
  class TaskRunner(
...
1. **CoarseGrainedExecutorBackend**接收到**CoarseGrainedSchedulerBackend**发送过来的**LaunchTask**消息
2. 在**CoarseGrainedExecutorBackend**内部，首先会对**LaunchTask**消息进行反序列化，重新得到序列化之前的**TaskDescription**对象
3. 反序列化出来的**TaskDescription**会被**Executor**通过**launchTask()**方法发布任务Task
4. 在**Executor**的**launchTask**方法内部，首先会根据**TaskDescription**创建一个实现了**Runnable**接口的线程**TaskRuner**对象**tr**
5. 然后记录正在运行的任务
6. 最终通过线程池**threadPool**来执行**TaskRunner**对象**tr**



#### 总结

![未命名文件](source/%E5%A4%A7%E6%95%B0%E6%8D%AE%E5%AD%A6%E4%B9%A0%E4%B9%8B%E8%B7%AF/%E6%9C%AA%E5%91%BD%E5%90%8D%E6%96%87%E4%BB%B6-16334225169802.svg)



1. 编写的**Spark**程序，每遇到一个**Action**算子就会启动一个**Job**
2. 首先会进入**DAGScheduler**的**runJob**方法中，进一步触发**DAGScheduler**中的**submitJob()**方法
3. **submitJob()**方法内部会将**rdd**封装成实现了**DAGSchedulerEvent**类的**JobSubmitted**事件，添加到**DAGSchedulerEventProcessLoop**中的**eventQueue**中
4. **DAGSchedulerEventProcessLoop**中会有一个事件线程**EventThread**，**run**方法就会从阻塞队列`EventQueue`中取出相应的事件进行运行。
5. 取出来的事件会通过**handleJobSubmitted()**方法进行**阶段的划分**和**阶段的提交**处理
6. 在**handleJobSubmitted**方法内部，首先会定义一个**ResultStage**类型的变量**finalStage**，在真正赋值之前会先创建其父阶段（如果父阶段存在），每个**stage**对象都会记录其**父阶段**的信息**Parent**
7. **finalStage**创建完成之后，通过**submitStage()**方法提交**ResultStage**阶段
8. submitStage()方法内部，首先会通过**getMissingParentStages()**方法获取传递过来的阶段的父阶段，如果父阶段存在，则递归的提交父阶段**submitStage(parentStage)**,如果不存在父阶段，就通过**submitMissingTasks(...)**进行**阶段中任务的切分与提交**
9. 在**submitMissingTask()**方法中，会根据传递过来的**stage**的类型创建相应类型的**Task**
   - 如果**Stage**的类型是**ShuffleMapStage**，就创建**ShuffleMapTask**类型的**Task**
   - 如果**Stage**的类型是**ResultStage**，就创建**ResultTask**类型的**Task**
10. 一个阶段中的**Tasks**创建完成后，会被封装成**TaskSet**对象
11. 通过**taskScheduler.submitTask()**方法传递给**TaskScheduler**进行任务级别的调度
12. **TaskScheduler**的**submitTask()**方法接收到**TaskSet**后，会把**TaskSet**进一步封装成**TaskSetManager**对象（负责监控任务的执行状态，失败重试等），然后根据**FIFO（默认）**或者**Fair策略**加入到资源调度池**rooPool**中
13. 每调用一次**submitTask**方法，就会相应的执行一次**backend.reviveOffers()**方法
14. 集群下对应的**SchedulerBackend**为**CoarseGrainedSchedulerBackend**，其中**reviveOffers()**方法内部会通过**driverEndPoint.send(ReviveOffers)**给自己发送**ReviveOffers**消息
15. **CoarseGrainedSchedulerBackend**收到传来的**ReviveOffers**类型的消息后，触发**makeOffers()**方法
16. **makeOffers()**方法首先会过滤出目前还存活的**Executor**，并将存活的**Executor**封装成**WorkOffers**对象**workOffers**，之后调用`taskScheduler.resourceOffers(workOffers)`
17. 随后会通过**TaskScheduler**中的**resourceOffers(workOffers)**，将资源调度池rootPool中的TaskSetManager按照特定的**调度算法FIFO(默认)或者Fair**取出，将其中的**Task**和**WorkOffers**对象结合任务的**本地化调度级别**，封装成**TaskDescription**对象，并将其**TaskDescription**进行返回到上述**SchedulerBackend**的**makeOffers()**中，继续向下执行
18. 接下来会调用**launchTask()**方法进行**Task的发布**
19. 在**launchTask()**方法内部会将相应的**TaskDescription**进行**序列化**，然后将**序列化后的TaskDescription封装成LaunchTask**消息
20. 封装好的**LaunchTask消息**会通过**executorEndPoint.send()方法**发送给远程机器相应的**Executor**上
21. **CoarseGrainedExecutorBackend**中的**receive()**方法负责接收远程的调度后端**CoarseGrainedSchedulerBackend**发送来的消息，其中首先会创建Executor对象，用于执行相应的任务
22. **CoarseGrainedExecutorBackend**接收到**CoarseGrainedSchedulerBackend**发送过来的**LaunchTask消息**，会对该消息进行**反序列化**，得到相应的**TaskDescription**
23. 然后通过**executor.launchTask(...)**方法发布相应序列化后的任务**TaskDescription**到对应的**Executor对象**中
24. 在具体的**Executor对象**中，会根据**launchTask()**方法接收到的**Task**创建一个实现了**Runnable接口**的**TaskRunner**对象**rn**，并将该运行中的任务记录到runningTasks中做记录
25. 最终通过**线程池threadPool**的方式，运行上述**rn**对象，其具体的实现细节定义在了**rn**的**run()**方法中
...
作者：Pure201911132215663
链接：https://www.nowcoder.com/discuss/737802?source_id=discuss_experience_nctrack&channel=-1
来源：牛客网



表sal_sum，每次统计前三个月的[销售]()额    
 month    sal    
 1       5    
 2       3    
 3       5    
...
select
		month
		sum(sal) over(order by month rows between 2 preceding and current row)	as sal_3_sum
from	sal_sum
```
...
2. 在集群正常工作之前，`myid`小的服务器给`myid`大的服务器投票，知道集群正常工作，选出`leader`；
...
3. 当Leader收到**大多数Server数据写成功了**（内部协议：**ZAB**），那么就说明数据写成功了。写成功之后，Leader会告诉Server1数据写成功了。
...
由于生产者生产的消息会不断追加到log文件末尾，为防止log文件过大导致数据定位效率低下，kafka采取了**切片**和**索引**机制，将每个partition分为多个**segment**。每个segment对应两个文件：`.index`和`.log`。这些文件位于一个文件夹下，该文件命名规则为：`topic名称+分区序号`。
...
**分区leader副本的选举**由**控制器**负责具实施。当创建分区、分区上线这些时候，都需要执行leader的选举动作。
...
- ack设置为0，不等待broker是否落盘成功，生产者继续发送数据。
- 生产者端配置`batch.size`参数，一次按照batch发送，默认16KB
- `linger.ms`,这个参数控制：当消息不够一个批次的时候，最长等待时间，如果超过这个时间就算每个达到`batch.size`,也会发送数据到`broker `
...
- flink故障恢复机制的核心
- 有状态流应用的一致性检查点，其实就是所有任务的**状态**，在某个时间点的一份快照，这个时间点，应该是所有任务都恰好处理完一个相同的输入数据的时候。

#### 检查点保存（Checkpoint）

做checkpoint的时候，状态后端是按照一组一组进行保存的，当所有的任务都恰好处理完一个相同的输入数据的时候，就将所有任务的<font color=green>当前状态进行保存</font>（**而不是保存正在处理的数据**）。

 ![img](source/2021年8月份秋招复习笔记/src=http%3A%2F%2Foscimg.oschina.net%2Foscnet%2Fc15e5cf0-5f5a-48ac-ba14-b3d370a196cb.png&refer=http%3A%2F%2Foscimg.oschina.net&app=2002&size=f9999,10000&q=a80&n=0&g=0n&fmt=jpeg)

如上图：状态的保存时机是source，sum_even和sum_odd这三个任务都处理完5这个数据的时间点，将这些任务的状态进行保存。

如果不能保证所有任务都处理完相同的输入，可能会出现以下情况：

source处理完5，但是由于网络原因，4,3都在堵塞中，导致sum_event和sum_odd还没有处理出完3,4,5这三个数据。如果此时发生故障，在进行故障恢复的时候，就会丢失数据。
...
如上述图：当处理到7这个数据的时候，sum_odd任务所负责的分区挂了，此时就需要做检查点恢复。

- 在执行流应用程序期间，Flink会定期保存状态的一致检查点。
- 如果发生故障，Flink将会使用最近的检查点来一致恢复应用程序的状态，并重新启动处理流程。

这就需要数据源能够有<font color=red>数据重放的功能</font>

![img](source/2021年8月份秋招复习笔记/url=http%3A%2F%2Fdingyue.ws.126.net%2F2021%2F0727%2F1e1f12b7j00qwwn0g002ac000hs00umg.jpg&thumbnail=650x2147483647&quality=80&type=jpg)



1. 遇到故障之后，第一步就是重启应用
2. 第二步是从checkpoint中读取状态，将状态重置
3. 从检查点重启应用程序后，其内部状态与检查点完成时的状态完全相同。
4. 第三步：开始消费并处理检查点到发生故障之间的所有数据
5. 这种检查点的保存和恢复机制可以为应用程序状态提供（Exactly-one）的一致性，因为所有算子都会保存检查点并恢复其所有状态，这样一来所有的输入流都会被重置到检查点完成时的位置。



关于Flink实现Exactly-Once的几点要求：

- source端支持数据重放的功能，当发生故障的时候，source可以进行故障之前的数据重放
- Flink内部通过CheckPoint保证发生故障时，状态的重置。
- sink端需要有**幂等性**或者**事务性**（日志预写和两阶段提交2PC）的支持。

#### CheckPoint算法

其核心就是<font color=red>barrier对齐</font>

其本质是基于Chandy-Lamport算法的分布式快照算法的改进。

将检查点的保存和数据处理分开，不暂停整个应用

![在这里插入图片描述](source/2021年8月份秋招复习笔记/i=20210111152147546.png)

- 现在是一个两个输入流的应用程序，用并行的两个source任务来读取

![在这里插入图片描述](source/2021年8月份秋招复习笔记/i=2021011115251435.png)



- JobManager会向每个Source任务发送一条带有检查点ID的消息，通过这种方法来启动检查点
- 这个消息会传递给下游所有任务



![在这里插入图片描述](source/2021年8月份秋招复习笔记/i=20210111153027619.png)

- 数据源将它们的状态写入检查点，并发出一个检查点barrier，广播发到下游任务。状态后端在状态存入检查点之后，会返回通知给source任务，source任务就会向jobmanager确定检查点完成。

![在这里插入图片描述](source/2021年8月份秋招复习笔记/i=20210111153426447.png)

- 分界线对齐（barrier对齐）：barrier向下游传递，sum任务会等待所有输入分区的barrier到达。

- 对于barrier已经到达的分区，继续接收到的数据会被缓存起来；而barrier尚未到达的分区，数据会被正常处理。

![在这里插入图片描述](source/2021年8月份秋招复习笔记/i=20210111153926877.png)

- 当接收到所有分区的barrier时，任务就将其状态保存到状态后端的检查点中，然后barrier继续向下游转发。

![在这里插入图片描述](source/2021年8月份秋招复习笔记/i=2021011115420988-16300710261841.png)

- 向下游转发barrier后，任务继续正常的进行数据处理

![在这里插入图片描述](source/2021年8月份秋招复习笔记/i=20210111154304922.png)

- sink任务想JobManager确认状态保存到checkpoint完毕
- 当所有任务都确认已经成功将状态保存到检查点时，检查点就真正完成了。

每个任务都有相应的状态的检查点，所有任务的状态checkpoint被状态后端state backend保存，当出现故障的时候哦，从此checkpoint就可以恢复任务。
...
- **MemoryStateBackend**
  ...
  内存级的状态后端，会将**键控状态**作为内存中的对象进行管理，将它们存储在TaskManager的JVM堆上 ；而将Checkpoint存储在JobManager的内存中。
...
- **at-most-once**: 这其实是没有正确性保证的委婉说法——故障发生之后，计数结果可能丢失，同样的还有UDP。（至多一次）
- **at-least-once**：这表示计数结果可能大于正确值，但绝不会小于正确值。也就是说，计数程序在发生故障后可能多算，但是绝不会少算。（至少一次）
- **exactly-once**：这指的是系统保证在发生故障后得到的计数结果与正确值一致。（精确一致）

Flink的一个重大价值在于：**它既保证了exactly-once，也具有低延迟和高吞吐的处理能力。**

#### 端到端（end to end）状态一致性

- flink内部保证——依赖checkpoint
- source端——需要外部数据源可重放数据（可重置数据的读取位置）
- sink端——需要保证从故障恢复时，数据不会重复写入外部系统。
  - 对于sink端，又有两种具体的实现方式：**幂等性写入**和**事务性写入**
  - **幂等性写入**：就是说一个操作，可以重复执行很多次，但只导致一次结果更改，也就是说，后面再重复执行就不起作用了。
  - **事务写入**：需要构建事务来写入外部系统，构建的事务对应着checkpoint，等到checkpoint真正完成的时候，才把所有对应的结果写入sink系统中。
    - 对于事务写入，具体又有两种实现方式：预写日志（WAL）和两阶段提交（2PC）。
...
在计算机系统中，更多的是通过关系型数据库来控制事务，这是利用数据库本身的事务特性来实现的，因此叫数据库事务。由于应用主要靠关系数据库来控制事务，而数据库通常和应用在同一个服务器，所以**基于关系型数据库的事务又称为本地事务**

<u>数据库事务在实现时会将一次事务设计的所有操作全部纳入到一个不可分割的执行单元，执行单元中的所有操作要么成功，要么都失败，只要其中任一操作执行失败，都将导致整个事务的回滚</u>

### 2.分布式事务

分布式系统会把一个应用系统拆分为可独立部署的多个服务，因此需要服务与服务之间远程协作才能完成事务操作，这种分布式系统环境下由不同的服务之间通过网络远程协作完成事务称之为**分布式事务**，例如用户注册送积分事务，创建订单减少库存事务，银行转账事务等都是分布式事务。
...
<font color=green>说白了就是主从之间的数据一致性</font>

**如何实现一致性？**

- 写入主数据库后要将数据同步到从数据库
- 写入主数据库后，再向从数据库同步期间要将从数据库锁定，待同步完成后再释放锁，以免在新数据写入成功后，向从数据库查询到旧的数据。
...
可用性是指任何事物操作都可以得到相应结果，且不会出现响应超时或响应错误。

**如何实现可用性？**

1. 写入主数据库后要将数据同步到从数据库
2. 由于要保证从数据库的可用性，不可将从数据库中的资源进行锁定
3. 即使数据还没有同步过来，从数据库也要返回要查询到的数据，哪怕是旧数据，如果旧数据也没有则可以按照约定返回一个默认信息，但不能返回错误或响应超时。
...
1. 尽量使用异步代取同步操作，例如使用异步方式将数据从主数据库同步到从数据库，这样节点之间能有效的实现松耦合。
2. 添加从数据库节点，其中一个节点挂了其他从节点提供服务



<font color=red>分区容忍性是分布式系统的基本能力</font>
...
**在所有分布式事务场景中不会同时具备CAP三个特性，因为在具备P的前提下C和A是不能共存的**
...
BASE是Basically Available（基本可用）、Soft state（软状态）和Eventually consistent（最终一致性）三个短语的缩写。BASE理论是对CAP中AP的一个扩展，通过牺牲强一致性来获得可用性，当出现故障允许部分不可用但要保证核心功能可用，允许数据在一段时间内是不一致的，但最终达到一致状态。满足BASE理论的事务，称之为**柔性事务**

### 4. 分布式事务解决方案☞2PC（两阶段提交）

针对不同的分布式场景，业界常见的解决方案有2PC，TCC可靠消息最终一致性、最大努力通知这几种
...
> 举例
>
> 张三和李四好久不见，老友约起聚餐，饭店老板要求先买单，才能出票。这时张三和李四分别抱怨近况不如意，囊中羞涩，都不愿意请客，这时只能AA。只有张三和李四都付款老板才能出票安排就餐。但由于张三和李四都是铁公鸡，形成了尴尬的一幕：
>
> - 准备阶段：老板要求张三付款，张三付款；老板要求李四付款，李四付款。
> - 提交阶段：老板出票，两人纷纷落座就餐。
>
> 例子中形成了一个事务，若张三和李四其中一人拒绝付款，或钱不够，店老板都不会给出票，并且会把已收款退回。



整个事务过程由<font color=red>事务管理器</font>和<font color=red>参与者</font>组成

店老板就是事务管理器；张三和李四就是参与者；事务管理器负责决策整个分布式事务的提交和回滚，事务参与者负责自己本地事务的提交和回滚。

在计算机部分关系数据库如MySql支持两阶段提交协议：

1. 准备阶段：事务管理器给每个参与者发送Prepare消息，每个数据库参与者在本地执行事务，并写本地的Undo/Redo日志，此时事务没有提交

   (Undo日志是记录修改前的数据，用于数据库的回滚；Redo日志是记录修改后的数据，用于提交事务后写入数据文件）

2. 提交阶段：如果**事务管理器**收到了**参与者的执行失败或者超时消息时**，直接给每个参与者发送回滚（Rollback）消息；否则，发送提交（Commit）消息；参与者根据事务管理器的指令执行提交或者回滚操作，并释放事务处理过程中使用的锁资源。<font color=red>注意：必须在最后阶段释放锁资源。</font>



![img](https://gimg2.baidu.com/image_search/src=http%3A%2F%2Fattach.dataguru.cn%2Fattachments%2Fforum%2F201509%2F20%2F084322huqeon7nkwtakmwq.png&refer=http%3A%2F%2Fattach.dataguru.cn&app=2002&size=f9999,10000&q=a80&n=0&g=0n&fmt=jpeg?sec=1633742797&t=84d69a96c3939ab09772d54e6a7613b6)

如果任何一方参与者的本地事务执行失败了，那么事务管理器就会给其他参与者发送**Rollback**指令，是其他参与者进行**回滚**。
...
- RM：资源管理器，可以理解为事务的参与者
- TM：事务管理器，负责协调和管理事务，事务管理器控制着全局事务，管理事务生命周期，并协调各个RM。**全局事务**是指分布式处理环境中，需要操作多个数据库共同完成一个工作，这个工作即是一个全局事务。
...
- 需要本地数据库支持XA协议
- 资源所需要等到两个阶段结束才释放，性能较差。



#### 3.解决方案Seata方案

Seata是阿里中间件团队发起的开源项目Fescar，后更名为Seata，他是一个开源的分布式事务框架。

传统的2PC的问题在Seata中得到了解决，它通过对本地关系数据库的分支事务的协调来驱动完成全局事务，是工作在应用层的中间件。主要优点是性能较好，且不长时间占用连接资源，它以高效并且对业务0侵入的方式解决微服务场景下面临的分布式事务问题，他目前提供AT模式（即2PC）及TCC模式的分布式事务解决方案。

**Seata的设计思想如下：**

Seata的设计目标是其一是对业务无侵入，因此从业务侵入的2PC方案着手，在传统2PC的基础上演进，并讲解2PC方案面临的问题。

Seata把一个分布式事务理解成一个包含了若干**分支事务**的**全局事务**。全局事务的职责是协调其下管辖的分支事务达成一致，要么一起成功提交，要么一起失败回滚。此外，通常分支事务本身就是一个关系数据库的本地事务，下图全局事务与分支事务的关系图：

![image-20210909102257596](source/2021年8月份秋招复习笔记/image-20210909102257596-16311541787475.png)



与传统2PC的模型类似，Seata定义了3个组件来协议分布式事务的处理过程

![image-20210909102436921](source/2021年8月份秋招复习笔记/image-20210909102436921.png)



- Transaction Coordinate（TC）：**事务协调器**，它是独立的中间件，需要独立部署运行，它维护全局事务的运行状态，接收TM指令发起全局事务的提交与回滚，负责与RM通信协调各个分支事务的提交或回滚。
- Transaction Manager（TM）：**事务管理器**，TM需要嵌入应用程序中工作，它负责开启一个全局事务，并最终向**TC**发起**全局提交或全局回滚的指令**。
- Resource Manager（RM）：**控制分支事务**，负责分支注册、状态汇报，并且接收事务协调器TC的指令，驱动分支（本地）事务的提交和回滚。



Seata方案的每个本地或者分支事务在执行完毕之后就执行提交操作，回滚的时候，删除数据的方式进行回滚。
...
而两阶段提交方面，传统2PC无论第二阶段的决议是commit还是rollback，事务性资源的锁都要保持到Phase2完成才释放。而Seata的做法是在Phase1就将本地事务提交，这样就可省去Phase2持锁的时间，整体提高效率。
...
> 路径分析:
>
> ​	是指用户在APP中或者网站中的**访问路径**。为了衡量网站**优化的效果**或者**营销推广的效果**，以及了解**用户行为偏好**，时常要对访问路径进行分析。



> 桑基图：
>
> ​	是用于描述一组值到另一组值的流向的图表。比如下图的人口流动情况
>
...
> 边的右侧节点表示终止点：**Target**
>
> 桑基图需要提供每种页面，节点的跳转次数。每个流向或者跳转由 source 与 target 表示。
>
> - source：跳转起始页面（**不允许为空**）
> - target：跳转终到页面（**可为空**）
>
...
通常情况下我们以直观简单的思想去定义 source和target的时候，可能会存在source为null的业务场景，例如下述sql，将last_page_id作为source就存在null的情况：
...
| source                      | target      |
| --------------------------- | ----------- |
| <font color=red>null</font> | home        |
| home                        | search      |
| search                      | good_list   |
| good_list                   | good_detail |
| good_detail                 | login       |
| login                       | good_detail |
| good_detail                 | cart        |
| cart                        | trade       |
...
这种情况我们可以转变思路，可以将 **page_id作为 source**， **当前行的下一行的page_id作为target**，这样就避免了source为null的情况了。

```sql
select
	session_id
	,page_id														source
	,lead(page,1,null) over(partition by session_id order by ts) 	target
from dwd_page_log
where dt='2020-06-14'
```
...
| cart        | trade                       |
| trade       | payment                     |
| payment     | <font color=red>null</font> |

虽然解决了source不为null的问题，但是观察上述数据，`good_detail -> login, login -> good_detail` 存在环，此时可以为每次跳转增加跳转序号，根据跳转序号为每个页面增加页面序号。
...
| -------------- | --------------------------- | ---- |
| home1️⃣          | search2️⃣                     | 1    |
| search2️⃣        | good_list3️⃣                  | 2    |
| good_list3️⃣     | good_detail4️⃣                | 3    |
| good_detail4️⃣   | login5️⃣                      | 4    |
| login5️⃣         | good_detail6️⃣                | 5    |
| good_detail6️⃣   | cart7️⃣                       | 6    |
| cart7️⃣          | trade8️⃣                      | 7    |
| trade8️⃣         | payment9️⃣                    | 8    |
| payment9️⃣       | <font color=red>null</font> | 9    |
| home1️⃣          | mine2️⃣                       | 1    |
| mine2️⃣          | orders_unpaid3️⃣              | 2    |
| orders_unpaid3️⃣ | good_detail4️⃣                | 3    |
| good_detail4️⃣   | good_spec5️⃣                  | 4    |
| good_spec5️⃣     | comment6️⃣                    | 5    |
| comment6️⃣       | trade7️⃣                      | 6    |
| trade7️⃣         | payment8️⃣                    | 7    |
| payment8️⃣       | <font color=red>null</font> | 8    |

这样 环`good_detail -> login, login -> good_detail` 就变成了`good_detail4 -> login5, login5 -> good_detail6`, 环就不复存在啦！😊

```sql
-- 解决环的问题
    select
        concat(source,'：',step) 	source -- 其实页面添加id
        ,concat(target,'：',step+1) 	target -- 终止页面添加id
    from
    (
        -- 当前查询可将每个会话的跳转明细查询出来，包括页面的跳转次序
        select
            session_id
            ,page_id source
            ,lead(page_id,1,null) over(partition by session_id order by ts) target  -- 当前行的下一行page_id 作为target，没有下一行默认值为null
            ,row_number() over (partition by session_id order by ts) step -- 每次会话的跳转序号
        from
        (
            select
                last_page_id
...
在进行指标的开发时，总会涉及到很多个维度字段，在开发时，首先就要区分这些**维度字段** 与 **指标字段**，然后先不考虑维度问题`换句话说就是：所有维度的汇总值`，<font color=red>先把所有维度的总汇总指标值计算出来之后，再通过group by 维度字段 来进行维度的细分！</font>，这样做会比直接考虑维度要简单的多。
...
|          | channel          | 渠道                             |
| **指标** | uv_count         | 日活                             |
|          | duration_sec     | 页面停留时长（所有会话）         |
|          | avg_duration_sec | 一次会话，页面停留平均时长       |
|          | page_count       | 页面总浏览数（所有会话）         |
|          | avg_page_count   | 一次会话，页面平均浏览数         |
|          | sv_count         | 会话次数                         |
|          | bounce_count     | 跳出数：一次会话中只浏览一个页面 |
|          | bounce_rate      | 跳出数/会话次数                  |

1. 先不考虑维度，计算出总的会话指标值（最近一天的数据）

```sql
select
    count(distinct mid_id) uv_count
    ,sum(duration)/1000 duration_sec
    ,avg(duration)/1000 avg_duration_sec
    ,sum(page_count) page_count
    ,avg(page_count) avg_page_count
    ,count(distinct session_id) sv_count
    ,sum(if(page_count=1,1,0)) bounce_count
    ,sum(if(page_count=1,1,0)) / count(distinct session_id) bounce_rate
from
(
    select
        session_id -- 每次会话
        ,mid_id
        ,count(page_id) page_count
        ,sum(during_time)/1000 duration
    from
    (
        select mid_id       -- 设备id
             , last_page_id -- 上一页id
...
2. 添加维度
   ...
   1. 添加渠道维度
      ...
      (
          ...
          group by session_id,mid_id,channel
      ) t3
      group by channel
      ```

   2. 以此类推，添加其他维度字段，实现，将总的数据进行维度的划分。



### 同时求 近1天，近7天， 近30天的指标值（通过一个字段的不同取值来区分）

当我们在一张表中同时求近1天，近7天和近30天的指标累积值时，一般有两种思路：

1. 先按近1天的逻辑进行分区过滤，计算指标值，然后将相同的逻辑拷贝出3份，每份按照相应的分区进行过滤，包括近1天的分区，近7天的分区，近30天的分组，然后将这三部分的数据最终进行`union all`操作，即可完成。

   > 这种方式虽然比较直接，但是同样的逻辑需要复制出3份，当修改逻辑代码时，需要同时修改3份代码，容易出错。而且代码不简洁！

2. 可以通过UDTF函数（一近多出）的方式将基础表中的数据进行扩容，分别生成近1天，近7天和近30天的这3类数据，然后通过对不同类别的数据分别进行统计，完成指标值的计算。

   >  
   >
   > ![未命名文件 (1)](source/大数据学习之路/未命名文件 (1).png)
...
/*
    TODO 会话的划分
            每个mid_id 的上一页page_id为null的时候就是每次会话的开始
            会话划分，新增一个字段，表示会话的id ： mid_id  +  会话开始时间
        over() 中 存在order by， 则窗口中的数据行数是  上边界 到 当前行。
        last_value() 接收两个参数：一个是要选择的字段， 另一个参数是一个bool类型的，false：表示不跳过null， true表示跳过空值
...
select '2020-06-14'                                               dt               -- 统计日期
     ...
     , recent_days                                                                 -- 最近天数
     , channel                                                                     -- 渠道
     , count(distinct mid_id)                                     uv_count         -- 日活
     , sum(duration) / 1000                                       duration_sec     -- 页面停留总时长
     , avg(duration) / 1000                                       avg_duration_sec -- 一次会话 页面停留平均时长
     , sum(page_count)                                            page_count       -- 页面浏览总数
     , avg(page_count)                                            avg_page_count   -- 每次会话，平均页面浏览数
     , count(distinct session_id)                                 sv_count         -- 会话次数
     , sum(if(page_count = 1, 1, 0))                              bounce_count     -- 跳出数
     , sum(if(page_count = 1, 1, 0)) / count(distinct session_id) bounce_rate      -- 跳出率
from (
         select session_id -- 每次会话
              , channel
              , recent_days
              , is_new
              , mid_id
         ...
         from (
                  select t1.mid_id    -- 设备id
                       ...
                       , concat(t1.mid_id, '-', last_value(session_start_point, true)
                                                           over (partition by t1.recent_days,t1.mid_id order by ts)) session_id
                  from (
                           select mid_id       -- 设备id
                                , last_page_id -- 上一页id
                                , page_id      -- 本页id
                                ...
                                , if(last_page_id is null, ts, null) session_start_point
                           from dwd_page_log
                                    lateral view explode(array(1, 7, 30)) tmp as recent_days  -- 此处 通过UDTF函数将原始数据 扩容为原来的3倍
         ...
         group by session_id, mid_id, channel, is_new, recent_days
     ) t3
...
- 透明传输
  ...
  - `SOH  -->  EOT --> EOT` 通过转义字符转换帧头、帧尾

- 差错监测
  - 物理层只管传输比特流，无法控制是否出错
  - 数据链路层负责起”差错监测“的工作
  - **奇偶校验码**
    - 如果出错位置为偶数，那么就无法判断是否出错
  - **CRC**
...
![image-20210826150352497](source/2021年8月份秋招复习笔记/image-20210826150352497-16299614343404.png)

模2运算后的余数，添加到原始串的末尾



发送端会计算出上述的位串，发送给接收端，接收端也会做模2运算，来对比最终得到的余数是否为0，来判断信息在传输过程中，是否出错了。
...
![image-20210826152556164](source/2021年8月份秋招复习笔记/image-20210826152556164-16299627579376.png)

#### IP协议详解

- Ip协议使得复杂的实际网络变为一个虚拟互连的网络
- IP协议使得网络层可以屏蔽底层细节而专注网络层的数据转发
- <font color=red>IP协议解决了在虚拟网络中数据报传输路径的问题</font>
...
![image-20210830141834926](source/2021年8月份秋招复习笔记/image-20210830141834926-16303043159223.png)
...
![image-20210830141932789](source/2021年8月份秋招复习笔记/image-20210830141932789.png)

​	网络层：

- A发出目的地为C的IP数据报，<font color=red>查询路由表发现下一跳为E</font>
- A将数据报发送给E
- <font color=red>E查询路由表发现下一步跳为F</font>，将数据报发送给F
- <font color=red>F查询路由表发现目的地C直接连接</font>，将数据报发送给C
...
- E的数据链路层接收到数据帧，<font color=red>把帧数据交给网络层</font>
- E<font color=red>查询路由表，发现下一跳为F</font>
- E<font color=red>把数据报交给数据链路层，并告知目的MAC地址为F</font>
- E的数据链路层<font color=red>封装数据帧并发送</font>



- F的数据链路层接收到数据帧，把<font color=red>帧数据交给网络层</font>
- F<font color=red>查询路由表，发现下一跳为C</font>
- F<font color=red>把数据报交给数据链路层</font>，<font color=red>并告知目的MAC地址为C</font>
- F的数据链路层<font color=red>封装数据帧并发送</font>
...
![image-20210830143707374](source/2021年8月份秋招复习笔记/image-20210830143707374-16303054283854.png)

**ARP缓存表**

![image-20210830143741682](source/2021年8月份秋招复习笔记/image-20210830143741682-16303054635275.png)
...
当ARP缓存表中<font color=green>没有对应的IP地址与MAC地址的映射</font>的时候

![image-20210830144246148](source/2021年8月份秋招复习笔记/image-20210830144246148.png)
...
![image-20210830145003625](source/2021年8月份秋招复习笔记/image-20210830145003625.png)
...
- A发出目的地为C的IP数据报，<font color=red>查询路由表发现下一跳为E</font>
- A将IP数据报<font color=red>交给数据链路层(查询ARP缓存表，得到目的MAC地址)，并告知目的MAC地址是E</font>
- 数据链路层<font color=red>填充源MAC地址</font>A和<font color=red>目的MAC地址E</font>
- 数据链路层通过物理层将数据发送给E



- E的数据链路层接收到数据帧，<font color=red>把帧数据交给网络层</font>
- E<font color=red>查询路由表(RARP 根据MAC地址，查询出IP地址)，发现下一跳为F</font>
- E<font color=red>把数据报交给数据链路层，并告知目的MAC地址为F</font>
- E的数据链路层<font color=red>封装数据帧并发送</font>
...
![image-20210830150452674](source/2021年8月份秋招复习笔记/image-20210830150452674.png)

IP地址可分为两个部分：**网络号**、**主机号**

根据网络与主机号所占位数的不同，可以将IP地址分成：A类地址、B类地址、C类地址三种

| 分类    | 特点                                 |
| ------- | ------------------------------------ |
| A类地址 | 网络号：8位；主机号：24位；首位为0   |
| B类地址 | 网络号：16位；主机号：16位；首位为10 |
| C类地址 | 网络号：24位；主机号：8位；首位为110 |

<img src="source/2021年8月份秋招复习笔记/image-20210830150719139.png" alt="image-20210830150719139" style="zoom:80%;" />



|      | 最小网络号                                     | 最大网络号                               | 子网数 | 最小主机号 | 最大主机号  | 主机数量 |
| ---- | ---------------------------------------------- | ---------------------------------------- | ------ | ---------- | ----------- | -------- |
| A    | 0<br />(0000 0000)                             | 127<br />(0111 1111)                     | 2^7    | 0.0.0      | 255.255.255 | 2^24     |
| B    | 128.0<br />(1000 0000.\|<br />0000 0000)       | 191.255<br />(1011 1111.<br />1111 1111) | 2^14   | 0.0<br />  | 255.255     | 2^16     |
| C    | 192.0.0<br />(1100 0000.<br />0000 0000<br />) | 223.255.255                              | 2^21   | 0          | 255         | 2^8      |
...
- A类地址<font color=red>网络段全0（0000 0000） 表示特殊网络</font>
- A类地址<font color=red>网络段后7位全1（0111 1111:127）表示回环地址</font>
- B类地址<font color=red>网络段（1000 0000.0000 0000：:128.0）是不可使用的</font>

- C类地址<font color=red>网络段（192.0.0）</font>是不可使用的

![image-20210830153927092](source/2021年8月份秋招复习笔记/image-20210830153927092.png)



如何判断一个IP地址是哪一类的IP地址呢？

>将网络号的前8位取出来，转化成二进制，通过判断前3个二进制位，来判断
>
>0：A类
>
>10：B类
...
public class MergeSort1 {
    ...
    /**
     * 归并排序主要有两个重要的步骤：分+合
     * 分：即将数组分为一块块更小的数组，直至之后一个元素
     * 合：进行将两个小分区的数据进行合并排序
     * 整个过程中，将排序的数据存放到一个临时数组中temp。
     * 最终将temp数组中的数据赋值到原始数组中。
     *
     * @param array
     * @param left
     * @param right
    ...
    public static void merge(int[] array, int left, int mid, int right, int[] temp) {
        ...
        // 上述循环执行完成后，有可能会存在一个区间中的数据还有剩余数据
        // 处理区间剩余元素（因为会存在一个区间还有剩余数据）
        while (i <= mid) {
            temp[t] = array[i];
            t += 1;
            i += 1;
        ...
        // 上述循环处理完成后，temp数组中的数据即为合并完之后的有序的数据
        int leftTemp = left;
        t = 0;
        /* 限定当前区间的右边界 */
...
`Multi Paxos`是在多个`Accpetor`中选举出来一个`主节点`，然后所有的提案都会先经过这个`主 Accpetor`进行处理后，同步给其他`Accpetor`。
...
1. 自主学习能力强，有记笔记，写文档，CSDN博客，刷题的习惯。对于刷题这方面，维护了一个github库。
2. 会不定时的进行阶段性的总结，每做过的一个小任务，做完之后都会复盘，找到其中遇到的问题，下次再遇到如何避免。
3. 对于需求的开发，每次自己都会尝试给自己定一个排期，比如一个需求技术文档产出需要多长时间，任务开发需要多长时间、数据测试上线需要多长时间、看板配置需要多长时间。。。等等。
4. 沟通上，乐于交流，沟通，对于下派不懂的任务我会首先自己去思考怎么去做，会，然后把疑问汇总起来，请教身边的哥哥姐姐。



## 2. 面试谈薪资技巧

### 1. 面试官问你的期望薪资要求（期望薪资是多少？）

不要直接报出你的期望薪资，不要直接就报出具体的薪资，把问题抛回去：**咱们公司对我这个岗位的薪资预算大概是多少？**
...
<font color=red>就今年的行情啊，各家公司都在控制相应岗位的成本，但是我对我自己的能力和贡献值还是有信心的</font>

1. 一来，我有我所应聘的这个岗位的实习经历，从需求对接，技术评审到任务开发，数据测试，上线，看板配置，以及需求二次迭代，我自己都独自的完成过。
2. 二来，我对Spark SQL任务的有实际的调优经验，而且是独自完成，动手能力我还是很有自信的。
3. 三来，我平常学习工作会有**记笔记**和**复盘**的习惯，github上自己维护了一个**blog**，会定期的去把阶段性的学习成果push到上面，定期总结，回顾，还有重构（不同时期对待一个问题的看法会有所改变）。
4. 最后一点就是沟通方面，不管是与产品进行沟通，还是与身边的哥哥姐姐这些大佬进行沟通，毫无压力。而且酒量也不错，社交这方面也是强项。
...
## 2. 针对于实习项目，业务上缺乏自己独立的思考

### 2.1 可视化全平台产品运营数据分析

需求背景：统计各方向运营数据报表的访问信息，看清报表的实际使用情况。

#### 一期需求

一期需求只做数易可视化平台，乘客方向的运营报表统计。

在这个阶段，我自己有过独自思考的两个点：

1. **一个是对于可视化平台这块的，在日后的需求迭代中，有没有可能会扩展到其他平台？**

   - 针对于第一点，我这里独自做了一个集齐某滴内部所有可视化平台的流量数据的统一性建设，将埋点数据进行抽取，隔离出一张可视化全平台的报表流量DWD数据明细层。
   - 每类报表为其分配了特定的id进行区分，report_type

2. **第二点是有没有可能会扩展到除了乘客方向的其他业务方向？**

   - 这里需要说明 一点，每个平台配置好的报表会统一的配置到一个叫做某花筒的门户下，但是这个门户下没有相应报表的维度信息

   - 其他方向包括

     > 某司机；某特惠月；某网开台，某MP，某UT，某拼车，某安全，某决策，某乘客等9个方向

   - **短期方案**，是每个业务方向的报表维度信息会汇总到2个产品姐姐那边，然后最后会统一的按照我的dim层设计的字段，统一维护到线上的一个文档中，然后每天以全量的方式导入至dim维度层。

   - **长期方案**，产品方与某花筒平台的后台负责方进行报表维度数据的落表，这样就不需要我们手动来维护整个报表

#### 二期需求

从数易的可视化平台扩展到某易、某表王、某制化这3个可视化平台，业务方向从1个乘客方向，慢慢扩展到了：某司机；某特惠月；某网开台，某MP，某UT，某拼车，某安全，某决策，某乘客等9个业务方向。



### 2.2 全生态数据增长建设

需求：**某网约车在微信，支付宝，QQ等生态下均有相应的业务**

我主要做的工作是：**对于冒泡，订单的场景归因的建设**
...
对于可不可能扩展到其他生态这个问题，我独自设计了一个包括全生态下的小程序的流量数据明细层，然后通过分区字段进行区分。DWD明细层；会给表的说明信息打上生态的标签。

不管是哪个生态的场景归因，都依赖于这个统一的DWD层



#### 二期需求

生态从微信，扩展到 支付宝，QQ、银行等生态
...
1. 隔离原始数据，原始数据中可能会包含大量的敏感数据，类似于：身份证号，手机号，经纬度，
2. 减少重复开发，开发人员也会减少很多不必要的工作量
3. 在一定程度上也会解耦
4. 把复杂问题简单化
...
1. 有时候会比较粗心
   - 随意会在学习和工作中我会提前列好 to-do list ，以免遗漏任务
   - 会定期进行阶段性的总结和复盘，某个任务中遇到了哪些问题，如何解决的，下次如何避免
2. 记忆力不是很强，东西一学多了，之前的东西就会忘记
   - 自己搭建博客，定期将阶段性的学习成果push到上面，定期进行过回顾，或者知识梳理，重新编排等

## 8. 自身存在的优点

1. 乐于沟通，敢于沟通，不管哪个年龄段的人，都能聊到一起去
2. 业务上手能力比较快，热爱大数据这个方向
3. 具有一定的自学能力
4. 坚持一直是我这个人最大的优点
